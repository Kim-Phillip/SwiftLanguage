...
<br>
<br>
`a % b`에 대한 답을 결정하기 위해, `%`연산자는 다음 방정식을 계산합니다.<br>
`a` = (`b`X `quotient`) + `remainder`<br>
`quotient`는 a 안을 채울 b의 가장 큰 배수입니다.<br>
이 방정식에 `13`과 `6`을 집어 넣으면 다음을 산출합니다.<br>
`13` = (`6`X`2`) + `1`<br>
`a`가 음수 값일 때의 나머지 계산에도 똑같이 적용합니다.<br>
`-13` = (`6`X`-2`) + `-1` (`b`가 음수일 때는 `b`의 부호를 무시합니다.)<br>